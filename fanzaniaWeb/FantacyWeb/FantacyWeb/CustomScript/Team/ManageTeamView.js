...
function OnPageLoad() {
   ...
   // alert(Tid);
    ...
    $('#div_bottom').focus();
    var Tstat = Session_TournamentStatus;
    $('#Tournament_Status').val(Tstat);
    if (Tstat == "INPROGRESS") {
        $('#SubsLeft').html(Session_Subsleft);
    } else {
        $('#SubsLeft').html('<img src="/Assets/Icon/infinity.png" alt="" height="20px" width="20px" />');
    }

    $('#anc_autofill').css("display", "none");
    $('#anc_reset').css("display", "none");
    $('#NitroLeft').val(Session_NitroLeft);
    $('#AutoPilotLeft').val(Session_AutoPilotLeft);
    $('#PainKillerLeft').val(Session_PainKillerLeft);

    $('#NitroUsed').val(Session_NitroUsed);
    $('#AutoPilotUsed').val(Session_AutoPilotUsed);
    $('#PainKillerUsed').val(Session_PainKillerUsed);

    var nitro = $('#NitroLeft').val();
    var aCap = $('#AutoPilotLeft').val();
    var pk = $('#PainKillerLeft').val();
    var nitroUsed = Session_NitroUsed;
    var aCapUsed = Session_AutoPilotUsed;
    var pkUsed = Session_PainKillerUsed;

    if (nitro == 0) {
        $('#Chk_Booster').attr("disabled", "disabled");
        $("#Nitro_panel1").css("display", "none");
        $("#Nitro_panel2").css("display", "block");
    }
    else {
        $('#Chk_Booster').removeAttr("disabled");
        $("#Nitro_panel1").css("display", "block");
        $("#Nitro_panel2").css("display", "none");
    }

    if (aCap == 0) {
        $('#Chk_AutoCaptain').attr("disabled", "disabled");
        $("#ac_panel1").css("display", "none");
        $("#ac_panel2").css("display", "block");
    }
    else {
        $('#Chk_AutoCaptain').removeAttr("disabled");
        $("#ac_panel2").css("display", "none");
        $("#ac_panel1").css("display", "block");
    }

    if (pk == 0) {
        $('#Chk_PainKiller').attr("disabled", "disabled");
        $("#pk_panel1").css("display", "none");
        $("#pk_panel2").css("display", "block");
    }
    else {
        $('#Chk_PainKiller').removeAttr("disabled");
        $("#pk_panel2").css("display", "none");
        $("#pk_panel1").css("display", "block");
    }

    if (nitroUsed == 'True') {
        $('#Chk_Booster').prop("checked", true);
        $('#Chk_AutoCaptain').prop("checked", false);
        $('#Chk_PainKiller').prop("checked", false);
    } else if (aCapUsed == 'True') {
        $('#Chk_AutoCaptain').prop("checked", true);
        $('#Chk_Booster').prop("checked", false);
        $('#Chk_PainKiller').prop("checked", false);
    } else if (pkUsed == 'True') {
        $('#Chk_PainKiller').prop("checked", true);
        $('#Chk_AutoCaptain').prop("checked", false);
        $('#Chk_Booster').prop("checked", false);
    }
...
function ShowMatchList(Tid) {
 
    var uurl = '/Team/GetFutureMatchs';
    ...
    $.ajax({
        ...
        success: function (data) {
           // console.log(data);
            var defaultindex;
            var divs = '';
            var cnt = 0; var flag = 0; var activeclass = ''; var itemCnt = 0; var defaultFlag = false; var ActiveSlideNo = 1;
            var slideId = 1; 
            var BattingTeam1 = '';
            var BattingTeam2 = '';
         ...
         //   return;
            ...
            var team1 = data[0].Team1ShortName;
            var team2 = data[0].Team2ShortName;
            $('#team1').val(team1);
            $('#team2').val(team2);
            var teamthird = sessionWinnerPrediction;
            if (team1 == teamthird) {
          ...
          //  $('.team2').val(team2);
           ...
           // var teamfirst = document.getElementById('team1').value;
          //  var teamsecond = document.getElementById('team2').value;
           // var teamthird = sessionWinnerPrediction;
         //   if (teamfirst == teamthird) {
              //  $("#team1").attr('checked', 'checked');
          //  } else if(teamsecond == teamthird)
           // {
             //   $("#team2").attr('checked', 'checked');
           // } else
          //  {
            ...
            $.each(data, function (index, value) {
                ...
                if (cnt < 4) {
                    ...
                    if (value.Weather == "rain") {
                        seticon += '<img src="/Assets/Icon/rain.png" alt=""  style="float: right;top: -3px; position: absolute;right: 18px; width: 28px;" />';
                    } else if (value.Weather == "sunny") {
                        seticon += '<img src="/Assets/Icon/sunny.png" alt=""  style="float: right;top: -3px; position: absolute;right: 18px; width: 28px;"  />';
                    } else if (value.Weather == "snow") {
                        seticon += '<img src="/Assets/Icon/snow.png" alt=""  style="float: right;top: -3px; position: absolute;right: 18px; width: 28px;"  />';
                    }
                    else if (value.Weather == "cloudy") {
                        seticon += '<img src="/Assets/Icon/cloudy.png" alt=""  style="float:right;top: -3px; position: absolute;right: 18px; width: 28px;"  />';
                    } else if (value.Weather == "thunderstorms") {
                        seticon += '<img src="/Assets/Icon/thunderstorms.png" alt=""  style="float:right;top: -3px; position: absolute;right: 18px; width: 28px;"  />';
                    }
                    ...
                    //    BattingTeam2 = '';
                    //}
                    cBox += '<div class="col-md-3"><a href="#1">'
                    cBox += '<div id="match_' + itemCnt + '" class="itemMatch-view ' + activeclass + '"><div class="icon-box-view">'
                    ...
                    if (value.MatchStatus == 'COMPLETE' || value.MatchStatus == 'FINISH' || value.MatchStatus == 'UPCOMING') {
...
function ClearFilter() {
    ...
    var Tid = Session_TournamentId;
    var teamname = '';
    grid.reload({ page: 1, TournamentId: Tid, FilterTeams: teamname });
}
...
function FilterTeam() {
    ...
    var Tid = Session_TournamentId;
    grid.reload({ page: 1, TournamentId: Tid, FilterTeams: filters });
...
function SaveCaptainVsCaptain() {
    ...
    var uurl = '/Team/SaveCaptainVsCaptain';
...
function SavePlayers_Confirm() {
    ...
    var NitroUsed = $('#NitroUsed').val();
    var AutoPilotUsed = $('#AutoPilotUsed').val();
    var PainKillerUsed = $('#PainKillerUsed').val();
    if (NoOfPlayer == 11) {
                        ...
                        if (CName == '') {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    var powerPlayText = ''
                                    if (NitroUsed == 'True') {
                                        powerPlayText = 'Nitro Power Play Enabled';
                                        $("#powerPlayImg").attr("src", "/Assets/Icon/Nitro.png");
                                    }
                                    else if (PainKillerUsed == 'True') {
                                        powerPlayText = 'PainKiller Power Play Enabled';
                                        $("#powerPlayImg").attr("src", "/Assets/Icon/Painkiller.png");
                                    }
                                    else if (AutoPilotUsed == 'True') {
                                        powerPlayText = 'Ultracaptain Power Play Enabled';
                                        $("#powerPlayImg").attr("src", "/Assets/Icon/Autocaptain.png");
                                    }
                                    else {
                                        powerPlayText = 'No Powerplay Enabled';
                                        $("#powerPlayImg").attr("src", "");
                                    }

                                    var tstat = $('#Tournament_Status').val();
                                    var PTran = 'You have unlimited transfers at this stage of the tournament.';
                                    var tcnt = data.TransferCnt
                                    if (tstat != 'UPCOMING' && tcnt != 0) {
                                        if (tstat == 'BREAK') {
                                            PTran = 'You have unlimited transfers at this stage of the tournament.'
                                        } else {
                                            PTran = 'Total transfers made since last match = <b>' + data.TransferCnt + '</b>'
                                        }
                                    }
                                    else if (tstat == 'INPROGRESS' && tcnt == 0) {
                                        PTran = 'Total transfers made since last match = <b>' + data.TransferCnt + '</b>'
                                    }
                                    else if (tstat != 'INPROGRESS' && tcnt == 0) {
                                        PTran = 'You have unlimited transfers at this stage of the tournament.'
                                    ...
                                    $('#TranCnt').html(PTran);
                                    $("#powerPlayMsg").html(powerPlayText);
...
function SavePlayers() {
    ...
    //if (document.getElementById('team1').checked) {
    // var team = document.getElementById('team1').value;
    //}else if (document.getElementById('team2').checked) {
    //  var team = document.getElementById('team2').value;
    //}
    var getvalue1 = $("#basic-switch-value").text();
    //alert(getvalue1);
   // return;
    var uurl = '/Team/SavePlayer';
    var pData = { NitroUsed: NitroUsed, PainKillerUsed: PainKillerUsed, AutoPilotUsed: AutoPilotUsed, WinnerPrediction: getvalue1 };
    var jsonData = JSON.stringify(pData);
    $.ajax({
        url: uurl,
        type: 'POST',
...
function BoosterChange(checkType) {
    var nitroleft = Session_NitroLeft;
    var aCapleft = Session_AutoPilotLeft;
    var pkleft = Session_PainKillerLeft;

    if (checkType == 'Chk_Booster') {
        if (!$('#Chk_Booster').is(':checked')) {
            $('#NitroUsed').val('False');
            if (pkleft > 0) {
                $('#Chk_PainKiller').removeAttr("disabled");
            }
        ...
        if (!$('#Chk_Booster').is(':unchecked')) {
            $('#NitroUsed').val('True');
            $('#PainKillerUsed').val('False');
            $('#AutoPilotUsed').val('False');
            $('#Chk_PainKiller').prop("checked", false);
            $('#Chk_AutoCaptain').prop("checked", false);
        }
    }
    if (checkType == 'Chk_PainKiller') {
        if (!$('#Chk_PainKiller').is(':checked')) {
            $('#PainKillerUsed').val('False');
            if (nitroleft > 0) {
                $('#Chk_Booster').removeAttr("disabled");
            }
        ...
        if (!$('#Chk_PainKiller').is(':unchecked')) {
            $('#PainKillerUsed').val('True');
            $('#NitroUsed').val('False');
            $('#AutoPilotUsed').val('False');
            $('#Chk_Booster').prop("checked", false);
            $('#Chk_AutoCaptain').prop("checked", false);
    ...
    if (checkType == 'Chk_AutoCaptain') {
        if (!$('#Chk_AutoCaptain').is(':checked')) {
            $('#AutoPilotUsed').val('False');
            if (pkleft > 0) {
                $('#Chk_PainKiller').removeAttr("disabled");
            }
        ...
        if (!$('#Chk_AutoCaptain').is(':unchecked')) {
            $('#AutoPilotUsed').val('True');
            $('#PainKillerUsed').val('False');
            $('#NitroUsed').val('False');
            $('#Chk_Booster').prop("checked", false);
            $('#Chk_PainKiller').prop("checked", false);
...
function MovefromList(pid) {
}

var grid;
function LoadPlayerlist(Tid) {
    grid = $("#playergrid").grid({
        dataSource: { url: '/Team/GetPlayerlist', data: { TournamentId: Tid } },
    ...
    grid.on("rowDataBound", function (e, $row, id, record) {
        ...
        if (record.PlayerType == 'overseas') {
            $row.find('.Player').html('<a  href="#" onclick="return showplayer12(\'' + record.PlayerId + '\',\'' + record.ParticipationTeamId + '\');"><span style="color: blue;" class="plyer-list-name" data-toggle="tooltip" data-placement="top" title="' + record.PlayerDesc + '">' + record.PlayerShortName + '</span><img src="/Assets/Icon/Overseas.png" alt="" height="20px" width="20px"/></a>');
        } else {
            $row.find('.Player').html('<a  href="#" onclick="return showplayer12(\'' + record.PlayerId + '\',\'' + record.ParticipationTeamId + '\');"><span style="color: blue;" class="plyer-list-name" data-toggle="tooltip" data-placement="top" title="' + record.PlayerDesc + '">' + record.PlayerShortName + '</span></a>');
        }
        $row.find('.Action').html('<a href="#" title="Add" onclick="return SelectOrRemove(\'' + record.PlayerId + '\',\'A\',\'' + record.PlayerSpeciality + '\',\'' + record.PlayerValue + '\',\'' + record.TeamDescription + '\',\'' + record.PlayerType + '\',\'' + record.TeamCapt + '\',\'' + record.TeamVCapt + '\');" ><img src="/Assets/Icon/plus.png" alt="" height="18px" width="18px"/></a>');
...
function showplayer12(PlayerId,ParticipationTeamId) {
    var pData = { ParticipationTeamId: ParticipationTeamId, PlayerId: PlayerId, APIPId: 272450, MatchId:0 };
    //console.log(pData);
   // return;
    var jsonData = JSON.stringify(pData);
    $.ajax({
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.result.status == "Success") {
                ...
                $('#SubsLeft').html(data.cnt.Transfer_left);
                var pcnt = $('#SelectedPlayerCnt').val();
                $('#Transfer_Cnt').val(data.cnt.Transfer_left);
                $('#spinnerContainer').hide();
            }
...
function LoadSelectedPlayers() {
    TeamDet.length = 0;
    var uurl = '/Team/LoadSelectedPlayers';
    $.ajax({
        url: uurl,
        type: 'POST',
        dataType: 'json',
        ...
        success: function (data) {
            if (data.length == 0) {
                $('#anc_autofill').css("display", "block");
                $('#anc_reset').css("display", "none");
            }
            else {
                $('#anc_reset').css("display", "block");
                $('#anc_autofill').css("display", "none");
            }
...
function ResetConfirmation() {
    $('#ModalResetConfirmation').modal('show');
}

function ResetSelectedPlayerList() {
    $('#ModalResetConfirmation').modal('hide');
    $('#spinnerContainer').show();
    TeamDet.length = 0;
    var uurl = '/Team/TeamPlayer_LastCutoff';
    var pData = { Type: 'Reset' };
    var jsonData = JSON.stringify(pData);
    $.ajax({
        url: uurl,
        type: 'POST',
        ...
        success: function (data) {
            ...
            LoadPlayersOnGround(data.result);
            if ($('#Tournament_Status').val() == 'INPROGRESS') {
                $('#SubsLeft').html(data.cnt.Transfer_left);
                $('#Transfer_Cnt').val(data.cnt.Transfer_left);
            }
            
            var Tid = Session_TournamentId;
            grid.reload({ page: 1, TournamentId: Tid, PlayerType: '' });
            $('#spinnerContainer').hide();
        },
        error: function (req, status, error) {
            $('#spinnerContainer').hide();
...
function AutofillSelectedPlayerList() {
    $('#spinnerContainer').show();
    TeamDet.length = 0;
    var uurl = '/Team/AutoSelectTeam';
...
function LoadPlayersOnGround(data) {
    ...
    $.each(data, function (index, value) {
        ...
        var pPrice = value.PlayerValue; var pt = ''; var playerActive = '';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.result.length == 0) {
                $('#anc_reset').attr("disabled", "disabled");
            }
            else {
                $("#anc_reset").removeAttr("disabled");
            }
        },
        error: function (req, status, error) {
            toastr_warning("Opps! something went wrong. Try reload this page.");
...
function LoadTeamSelectionRules() {
    var uurl = '/Team/PlayerSelectionRules';
    var Tid = Session_TournamentId;
    var pData = { TournamentId: Tid };
    var jsonData = JSON.stringify(pData);
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (index, value) {
                ...
                var Tstat = Session_TournamentStatus;
                if (Tstat == 'INPROGRESS') {
                    $('#SubsCount').html('/' + value.SubCount);
                } else {
                    $('#SubsCount').html('');
                }
                $('#Rule_SubCount').val(value.SubCount);
