...
public class CreateLeagueDialog extends AlertDialog.Builder {
    private Context context;
    private ICreateLeaguDialog iCreateLeaguDialog;
    private JoinLeagueDialogBinding binding;
    private GradientDrawable bgShape;
    private AlertDialog alertDialog;

    public CreateLeagueDialog(Context context, ICreateLeaguDialog iCreateLeaguDialog, String tournamentID, String customerId) {
        super(context);
        ...
        binding.txtTitle.setText(context.getString(R.string.text_newleague));
        binding.btnYes.setText(context.getString(R.string.text_create));
        ...
        binding.btnYes.setOnClickListener(view -> {
            if (!binding.edtContent.getText().toString().trim().isEmpty()) {
                if (binding.edtContent.getText().toString().length()>2){
                    ...
                    CreateLeague(tournamentID, binding.edtContent.getText().toString(), customerId);
    ...
    public interface ICreateLeaguDialog {
        void createLeague(Boolean value);
    }

    private void CreateLeague(String tournamentID, String leagueName, String customerId) {

        Map<String, Object> map = new HashMap<>();
        map.put(Constants.TAG_LEAGUENAME, leagueName);
        map.put(Constants.TAG_TOURNAMENTID, tournamentID);
        map.put(Constants.TAG_LEAGUELEADERID, customerId);

        RetrofitAipService retrofitAipService = RetrofitClient.getInstance().create(RetrofitAipService.class);

        Call<JsonObject> call = retrofitAipService.CreateLeague(Constants.str_HEADER,
                ...
                if (response.body() != null) {
                    try {
                        ...
                        if (jsonObject.getString(Constants.str_status).equals(Constants.STR_SUCCESS)) {
                            alertDialog.dismiss();
                            iCreateLeaguDialog.createLeague(true);
